...
gerafichas<-function(coletas,qtdefichaspagina=6,qtdemaxpaginasjuncao=15,pastafichas,
                     LogoHerbario, nomeherbario, instituicao, projeto)
{
  nficha<-0
  nfile_Rdm<-0
  fichas=as.list({})
  ...
  for (i in 1:NROW(coletas)){
    ...
    # +1
    dup=ifelse(is.na(coletas$duplicateQuantity[i]),1,as.integer(coletas$duplicateQuantity[i])) 
    dup=ifelse(is.na(coletas$duplicateQuantity[i]==''),1,as.integer(coletas$duplicateQuantity[i])) 
    dup=ifelse(as.integer(coletas$duplicateQuantity[i]==0),1,as.integer(coletas$duplicateQuantity[i])) 
    ...
    for (d in 1:dup){
      ...
      nficha=nficha+1
      if(nficha==1){fichas=append(fichas,list(f1=carregaficha(coletas[i,],LogoHerbario, nomeherbario, instituicao, projeto)))}
      if(nficha==2){fichas=append(fichas,list(f2=carregaficha(coletas[i,],LogoHerbario, nomeherbario, instituicao, projeto)))} 
      if(nficha==3){fichas=append(fichas,list(f3=carregaficha(coletas[i,],LogoHerbario, nomeherbario, instituicao, projeto)))} 
      if(nficha==4){fichas=append(fichas,list(f4=carregaficha(coletas[i,],LogoHerbario, nomeherbario, instituicao, projeto)))} 
      if(nficha==5){fichas=append(fichas,list(f5=carregaficha(coletas[i,],LogoHerbario, nomeherbario, instituicao, projeto)))} 
      if(nficha==6){fichas=append(fichas,list(f6=carregaficha(coletas[i,],LogoHerbario, nomeherbario, instituicao, projeto)))} 
...
########################################################################
# Agrupa Fichas: em um ?nico arquivo ou varios com 100 
########################################################################
agrupafichas<-function(qtdefichaspagina=6,pastafichas,qtdemaxpaginasjuncao=15)
{
  ...
  for(p in 1:length(rmd)){
    ...
    #comparar com rmd e processar a difiren?a
    rmd.tmp[p]<-paste0(nfile_Rdm,'.Rmd')

    #####file_Rmd<-paste0(pastafichas,'/',rmd[p])
    
### Retirei isso em 15 06 18
### retornei em 03-07-18    
    
    # # 09-02-18 - else para imprimir uma p?gina 
     if (file.exists(file_Rmd)){
       txt <- paste0("```{r child = '", file_Rmd, "'}\n```\n")
       txt.Rmd=ifelse(!is.null(txt.Rmd),paste0(txt.Rmd,"\n",txt),txt)}
     else{
       file_Rmd <- list.files(pastafichas,pattern = '*.Rmd', include.dirs = FALSE)[1]
       txt <- paste0("```{r child = '", file_Rmd, "'}\n```\n")
       txt.Rmd=ifelse(!is.null(txt.Rmd),paste0(txt.Rmd,"\n",txt),txt)}
# ### ficou
#     file_Rmd <- list.files(pastafichas,pattern = '*.Rmd', include.dirs = FALSE)[1]
...
carregaficha<-function(coletas, LogoHerbario, nomeherbario, instituicao, projeto)
{
  return(data.frame(
    ...
    # 14-08-2018 - espaco apos cf
    cf<-  rep("", NROW(coletas)), #paste0(limpaNA(coletas$identificationQualifier),' '),
    
    nomecientifico <- paste0(limpaNA(coletas$genus), " ",limpaNA(coletas$specificEpithet), ifelse(!is.na(coletas$specificEpithet), paste0(" ", coletas$specificEpithet), "")),
    ...
    datadet<- limpaNA(coletas$dateIdentified), #montadata(limpaNA(coletas$dayIdentified),limpaNA(coletas$monthIdentified),limpaNA(coletas$yearIdentified)),
    pais<-limpaNA(coletas$country),
    municipio<-limpaNA(coletas$municipality),
    estado<-limpaNA(coletas$stateProvince),
    lat<-limpaNA(coletas$decimalLatitude),
    long<-limpaNA(coletas$decimalLongitude),
    alt<-limpaNA(coletas$maximumElevationInMeters),
    localizacao<-limpaNA(paste0(pais,', ',municipio,', ',estado,'.',' ( ',lat,' S, ',long,' W, ',alt,' m )')),
    
    localidade<-limpaNA(limpaNA(coletas$locality)),
    
    datacoleta<-limpaNA(coletas$eventDate),
    
    numerocoleta<-limpaNA(coletas$recordNumber),
    coletorprincipal<- paste0(limpaNA(coletas$recordedBy), ", ",  limpaNA(coletas$collectorInitials)),
    outroscoletores<-limpaNA(coletas$otherCollectors),
    coletorformatado<-formatacoletor(coletorprincipal,outroscoletores,numerocoleta),
    
    #numeroduplicatas<-limpaNA(coletas$dups),
    # quand dup = numero de fichas
    numeroduplicatas<-as.character(as.integer(limpaNA(coletas$duplicateQuantity))),
    
    duplicatasherbarios<-rep("", NROW(coletas)),
    
    notas<-limpaNA(coletas$fieldNotes),
    obs<-rep("", NROW(coletas))
    ))
}
########################################################################

########################################################################
# Pequenas func?oes de format??o e limpeza de dados
########################################################################
